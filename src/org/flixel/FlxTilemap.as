...
//			_buffer = new BitmapData(bw,bh,true,0);
		...
		protected function generateBoundingTiles():void
		{
			...
			if(!skipGen)
			{
				...
				_flashRect.x = 0;
				_flashRect.y = 0;
				_flashRect.width = (FlxU.ceil(FlxG.width / _tileWidth) + 1)*_tileWidth;
				_flashRect.height = (FlxU.ceil(FlxG.height / _tileHeight) + 1)*_tileHeight;
			}
		}
...
//			_buffer.fillRect(_flashRect,0);
			...
			if(tx < 0) tx = 0;
			if(tx > (widthInTiles-_screenCols)+_tileWidth)
			{
				tx = (widthInTiles-_screenCols)+_tileWidth;
				_flashPoint.x = 0;
			}
			if(ty < 0) ty = 0;
			
			if(ty > (heightInTiles-_screenRows)+_tileHeight)
			{
				ty = (heightInTiles-_screenRows)+_tileHeight;	
				_flashPoint.y = 0;
			}
			...
			var c:uint;
			var cri:uint;
			var startX:Number = _flashPoint.x %= _tileWidth;
			while(r < _screenRows)
			{
				cri = ri;
				c = 0;
				_flashPoint.x = startX;
				while(c < _screenCols)
				{
